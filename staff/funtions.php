...
function ubahStaff($data){
   ...
   if(empty($username)){
      echo"<script>alert('nama Staff tidak boleh kosong'); location.href='editProfile.php' </script>";
   }
   elseif(empty($email)){
      echo"<script>alert('email tidak boleh kosong'); location.href='editProfile.php' </script>";
   }
   elseif(empty($password)){
      $update = mysqli_query($connection,"UPDATE user set username ='".$username."', email ='".$email."', role ='".$role."' WHERE id= '".$id."'");
      if($update){
         echo"<script>alert('sukses mengubah staff'); location.href='../home_staff.php'</script>";
      } else {
         echo"<script>alert('gagal mengubah Staff'); location.href='../home_staff.php'</script>";
      }   
   } else {
